...
StarList::StarList(GraphList *g, int numberOfVertices, int numberOfEdges, int maxPortais) : g(g), numberOfVertices(numberOfVertices), numberOfEdges(numberOfEdges), maxPortais(maxPortais)
{
...
double StarList::run(int target)
{
  ...
  while (!priority_list.isEmpty())
  {
    ...
    while (neighbour)
    {
      ...
      if (dist[neighbour->vertex] > w)
      {
        ...
        double h = g->getVertex(neighbour->vertex).dist(g->getVertex(target));
        priority_list.insert({-w, -neighbour->vertex, pNeighbour, -h});
      }
      neighbour = neighbour->next;
    }
  }
