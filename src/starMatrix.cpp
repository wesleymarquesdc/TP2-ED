...
StarMatrix::StarMatrix(GraphMatrix *g, int numberOfVertices, int numberOfEdges, int maxPortais) : g(g), numberOfVertices(numberOfVertices), numberOfEdges(numberOfEdges), maxPortais(maxPortais)
{
...
double StarMatrix::run(int target)
{
  ...
  double h = g->getVertex(0).dist(g->getVertex(target));
  priority_list.insert({0, 0, 0, -h});
  // enquanto o heap não estiver vazio, pega a quádripla {distância, vértice, portais, heuristica} do heap (contém alguma distância possível de se chegar ao vértice partindo do início, usando uma quantidade de portais) e compara com a menor distância gravada que vai do início ao vértice. Se a distância nova for menor que a distância gravada, armazena a nova distância.
  while (!priority_list.isEmpty())
  {
    ...
    for (int j = 0; j < numberOfVertices; j++)
    {
      ...
      {
        // calcula distância do vértice para o target para a heurística (heap ordena com base na distância + heurística)
        double h = g->getVertex(neighbour[j].vertex).dist(g->getVertex(target));
        priority_list.insert({-w, -neighbour[j].vertex, pNeighbour, -h});
      }
    }
  }
  return INF;
